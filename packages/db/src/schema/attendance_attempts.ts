...
export const attendanceAttempts = pgTable(
  "attendance_attempts",
  {
    ...
    rawSessionRef: varchar("raw_session_ref", { length: 120 }),
  ...
  (table) => ({
    sessionIdIdx: index("attendance_attempts_session_id_idx").on(
      table.sessionId,
    ),
    scannedAtIdx: index("attendance_attempts_scanned_at_idx").on(
      table.scannedAt,
    ),
